"""
Reproduce test02.yaml: AAPM-like geometry, single sphere, SOD=570, SDD=1040.
...
YAML_PATH = os.path.join(os.path.dirname(__file__), "..", "tests", "test02.yaml")
...
# --- Generate phantom ---
print(f"[test02] Generating phantom {ps['rows']}x{ps['columns']}x{ps['slices']} ...")
...
# --- Forward projection ---
print("[test02] Forward projection with ASTRA ...", end=" ", flush=True)
t0 = time()
...
# --- Katsevich filtering ---
print("[test02] Katsevich filtering ...")
...
print("[test02] Backprojection (Katsevich weighted) ...")
t0 = time()
rec = backproject_safe(sino_td, conf, vol_geom, proj_geom, tqdm_bar=True)
print(f"done in {time()-t0:.2f}s")

# --- Save result slices ---
sl_indices = [phantom.shape[2] // 4, phantom.shape[2] // 2, phantom.shape[2] * 3 // 4]
fig, axes = plt.subplots(2, len(sl_indices), figsize=(12, 8))
fig.suptitle("test02: Phantom vs Katsevich Reconstruction (AAPM-like geometry)")
for i, si in enumerate(sl_indices):
    axes[0, i].imshow(phantom[:, :, si], cmap="gray")
    axes[0, i].set_title(f"Phantom z={si}")
    axes[0, i].axis("off")
    # Map phantom z-index to rec z-index (may differ in z dimension)
    si_rec = int(si * rec.shape[2] / phantom.shape[2])
    cs = axes[1, i].imshow(rec[:, :, si_rec], cmap="gray")
    axes[1, i].set_title(f"Recon z={si_rec}")
    axes[1, i].axis("off")
    fig.colorbar(cs, ax=axes[1, i], fraction=0.046)
plt.tight_layout()
out_path = os.path.join(OUT_DIR, "test02_result.png")
plt.savefig(out_path, dpi=150)
print(f"[test02] Saved -> {out_path}")
plt.close()
