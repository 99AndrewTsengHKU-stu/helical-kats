...
def main():
    ...
    if is_raw_mu:
        mu_water = auto_calibrate_mu_water(rec)
        # Sanity check: mu_water should be in [0.005, 0.05] for medical CT
        if not (0.005 < mu_water < 0.05):
            print(f"[WARNING] mu_water={mu_water:.5f} outside expected range, using 0.019")
            mu_water = 0.019
